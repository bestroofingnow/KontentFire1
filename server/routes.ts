...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auto-content/schedules/history', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const contentHistory = await db.select()
        .from(autoGeneratedContents)
        .where(and(
          eq(autoGeneratedContents.userId, user.id),
          sql`${autoGeneratedContents.status} IN ('published', 'failed')`
        ))
        .orderBy(sql`${autoGeneratedContents.publishedDate} DESC NULLS LAST, ${autoGeneratedContents.createdAt} DESC`);
  ...
  app.delete('/api/auto-content/schedule/:id', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const [schedule] = await db.select()
        .from(autoGeneratedContents)
        .where(eq(autoGeneratedContents.id, scheduleId))
      ...
      if (schedule.status === 'published') {
        return res.status(400).json({ message: 'Cannot cancel already published content' });
      }
