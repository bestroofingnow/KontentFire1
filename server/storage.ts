...
export class DatabaseStorage implements IStorage {
  ...
  async getUpcomingAutoContent(userId: number): Promise<AutoGeneratedContent[]> {
    ...
    return await db.select()
      ...
      .where(eq(autoGeneratedContents.userId, userId))
      .where(eq(autoGeneratedContents.status, 'pending'))
      .where(gte(autoGeneratedContents.scheduledDate, now))
  ...
  async getAutoContentHistory(userId: number): Promise<AutoGeneratedContent[]> {
    return await db.select()
      .from(autoGeneratedContents)
      .where(eq(autoGeneratedContents.userId, userId))
      .where(sql`${autoGeneratedContents.status} IN ('published', 'failed')`)
      .orderBy(desc(autoGeneratedContents.publishedDate || autoGeneratedContents.createdAt));
  }
